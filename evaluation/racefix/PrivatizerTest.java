...
public class PrivatizerTest extends DataRaceAnalysisTest {
	...
	public void beforeTest() throws ClassHierarchyException,
		...
		foundCA = findCA("Lracefix/PrivatizerSubject", name.getMethodName()
				+ "()V");
	...
	private SSAPutInstruction findInstructionForName(String string,
			List<CGNode> startNodes) {
		...
		for (CGNode cgNode : startNodes) {
			...
			for (SSAInstruction ssaInstruction : instructions) {
				...
				if (ssaInstruction instanceof SSAPutInstruction) {
					...
					if (put.getDeclaredField().getName().toString()
							.contains(string)) {
						startInstr = put;
						break;
					}
