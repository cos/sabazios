...
final class Line3D {
   ...
   * </p>
   * <p>
   * Note that this routine is only used for visibility checking. To avoid integer rounding errors which cause cracking
   * to occur in 'solid' surfaces, the lines are actually drawn from their original end-points.
   * 
   * The nuance is that this algorithm doesn't just deliver a boolean. It delivers the trimmed line. Although we need to
   * start the raster loop at the origin for good surfaces, we can save lots of time by saving the known endpoints as
   * globals variables. -- Bob Hanson 7/06
   * </p>
   * 
  ...
  int getTrimmedLine() { // formerly "visibilityCheck()"
    ...
    // System.out.println("trimmed line " + x1t + " " + y1t + " " + z1t + " " + x2t + " " + y2t + " " + z2t + " " + cc1
    // + "/" + cc2);
    return VISIBILITY_CLIPPED;
  }

  void plotLine(int argbA, boolean tScreenedA, int argbB, boolean tScreenedB, int xA, int yA, int zA, int xB, int yB,
      int zB, boolean clipped) {
    ...
    plotLineClipped(argbA, tScreenedA, argbB, tScreenedB, xA, yA, zA, xB - xA, yB - yA, zB - zA, clipped, 0, 0);
  }

  void plotLineDelta(int argbA, boolean tScreenedA, int argbB, boolean tScreenedB, int xA, int yA, int zA, int dxBA,
    ...
    plotLineClipped(argbA, tScreenedA, argbB, tScreenedB, xA, yA, zA, dxBA, dyBA, dzBA, clipped, 0, 0);
  }

  void plotLineDelta(int[] shades1, boolean tScreened1, int[] shades2, boolean tScreened2, int shadeIndex, int xA,
      int yA, int zA, int dxBA, int dyBA, int dzBA, boolean clipped) {
    ...
    plotLineClipped(shades1, tScreened1, shades2, tScreened2, shadeIndex, xA, yA, zA, dxBA, dyBA, dzBA, clipped, 0, 0);
  }

  void plotLineDeltaBits(int[] shades1, boolean tScreened1, int[] shades2, boolean tScreened2, int shadeIndex, int xA,
      int yA, int zA, int dxBA, int dyBA, int dzBA, boolean clipped) {
    ...
    if (clipped && getTrimmedLine() == VISIBILITY_OFFSCREEN)
      return;
    plotLineClippedBits(shades1, tScreened1, shades2, tScreened2, shadeIndex, xA, yA, zA, dxBA, dyBA, dzBA, 0, 0);
  }

  void plotDashedLine(int argb, boolean tScreened, int run, int rise, int xA, int yA, int zA, int xB, int yB, int zB,
      boolean clipped) {
    ...
    plotLineClipped(argb, tScreened, argb, tScreened, xA, yA, zA, xB - xA, yB - yA, zB - zA, clipped, run, rise);
  }

  private void plotLineClipped(int argb1, boolean tScreened1, int argb2, boolean tScreened2, int x, int y, int z,
      int dx, int dy, int dz, boolean clipped, int run, int rise) {
    ...
    // int test1 = (g3d.random > 0.9 ? -1 : 1);System.out.println("line3db " + test1 + " " + x + " " + y + " " + dx +
    // " " + dy);
  ...
  private void plotLineClipped(int[] shades1, boolean tScreened1, int[] shades2, boolean tScreened2, int shadeIndex,
      int x, int y, int z, int dx, int dy, int dz, boolean clipped, int run, int rise) {
    ...
    // int test1 = (g3d.random > 0.9 ? -1 : 0);System.out.println("line3d " + test1 + " " + x + " " + y + " " + dx + " "
    // + dy);
    ...
    if (dy <= dx) {
      ...
      for (int n = dx - 1, nMid = n / 2; --n >= n1;) {
        ...
        if (twoDxAccumulatedYError > dx) {
          offset += yOffsetIncrement;
          // sy=" n,ny="+n+"  accumulated error "+dx+"/"+twoDxAccumulatedYError+" yincr="+yOffsetIncrement+" offset " +
          // offset;
          twoDxAccumulatedYError -= twoDx;
          flipflop = !flipflop;
        }
  ...
  private void plotLineClippedBits(int[] shades1, boolean tScreened1, int[] shades2, boolean tScreened2,
      int shadeIndex, int x, int y, int z, int dx, int dy, int dz, int run, int rise) {
