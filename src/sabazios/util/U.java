...
public class U {
  ...
  public static int getValueForVariableName(CGNode node, String name) {
    SymbolTable symbolTable = node.getIR().getSymbolTable();
    int maxValueNumber = symbolTable.getMaxValueNumber();
    for (int i = 1; i <= maxValueNumber; i++) {
      String variableName = CodeLocation.variableName(node, i);
