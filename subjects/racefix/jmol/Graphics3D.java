 ...
 * 
 *         This is a two-pass rendering system. In the first pass, all opaque objects are rendered. In the second pass,
 *         all translucent objects are rendered.
 * 
 *         If there are no translucent objects, then that is found in the first pass as follows:
 * 
 *         The renderers first try to set the color index of the object to be rendered using setColix(short colix), and
 *         that method returns false if we are in the wrong pass for that type of object.
 * 
 *         In addition, setColix records in the boolean haveTranslucentObjects whether a translucent object was seen in
 *         the first pass.
 * 
 *         The second pass is skipped if this flag is not set. This saves immensely on rendering time when there are no
 *         translucent objects.
 * 
 *         THUS, IT IS CRITICAL THAT ALL RENDERING OPTIONS CHECK THE COLIX USING g3d.setColix(short colix) PRIOR TO
 *         RENDERING.
 * 
 *         Translucency is rendered only approximately. We can't maintain a full buffer of all translucent objects.
 *         Instead, we "cheat" by maintaining one translucent z buffer. When a translucent pixel is to be written, its z
 *         position is checked and...
 * 
 *         ...if it is behind or at the z position of any pixel, it is ignored ...if it is in front of a translucent
 *         pixel, it is added to the translucent buffer ...if it is between an opaque and translucent pixel, the
 *         translucent pixel is turned opaque, and the new pixel is added to the translucent buffer
 * 
 *         This guarantees accurate translucency when there are no more than two translucent pixels between the user and
 *         an opaque pixel. It's a fudge, for sure. But it is pretty good, and certainly fine for "draft" work.
 * 
 *         Users needing more accurate translucencty are encouraged to use the POV-Ray export facility for
 *         production-level work.
 * 
 *         Antialiasing is accomplished as full scene antialiasing. This means that the width and height are doubled
 *         (both here and in TransformManager), the scene is rendered, and then each set of four pixels is averaged
 *         (roughly) as the final pixel in the width*height buffer.
 * 
 ...
 * 
 *         The difference will be speed and memory. Adding translucent objects doubles the buffer requirement, and
 *         adding antialiasing quadruples the buffer requirement.
 * 
 *         So we have:
 * 
 ...
 * 
 *         Note that no antialising at all is required for POV-Ray output. POV-Ray will do antialiasing on its own.
 * 
 *         In principle we could save a bit in the case of antialiasing of just opaque objects and reuse the p and z
 *         buffers for the translucent buffer, but this hasn't been implemented because the savings isn't that great,
 *         and if you are going to the trouble of having antialiasing, you probably what it all.
 * 
 */
...
final public class Graphics3D implements JmolRendererInterface {
   ...
   *          for zShade
   */
  public void setSlabAndDepthValues(int slabValue, int depthValue, boolean zShade, int zSlab, int zDepth) {
  ...
  private void downsampleFullSceneAntialiasing(boolean downsampleZBuffer) {
    ...
    for (int i = windowHeight; --i >= 0; offset4 += width4)
      for (int j = windowWidth; --j >= 0; ++offset1) {
         ...
         * 
         * int a = pbuf[offset4]; int b = pbuf[offset4++ + width4]; int c = pbuf[offset4]; int d = pbuf[offset4++ +
         * width4]; int argb = ((((a & 0x0f0f0f) + (b & 0x0f0f0f) + (c & 0x0f0f0f) + (d & 0x0f0f0f)) >> 2) & 0x0f0f0f) +
         * ( ((a & 0xF0F0F0) + (b & 0xF0F0F0) + (c & 0xF0F0F0) + (d & 0xF0F0F0) ) >> 2);
         */

        int argb = ((pbuf[offset4] >> 2) & 0x3F3F3F3F) + ((pbuf[offset4++ + width4] >> 2) & 0x3F3F3F3F)
            + ((pbuf[offset4] >> 2) & 0x3F3F3F3F) + ((pbuf[offset4++ + width4] >> 2) & 0x3F3F3F3F);
  ...
  public void fillEllipsoid(Point3f center, Point3f[] points, int x, int y, int z, int diameter,
      Matrix3f mToEllipsoidal, double[] coef, Matrix4f mDeriv, int selectedOctant, Point3i[] octantPoints) {
    switch (diameter) {
    case 1:
      plotPixelClipped(argbCurrent, x, y, z);
      return;
    ...
    if (diameter <= (antialiasThisFrame ? Sphere3D.maxSphereDiameter2 : Sphere3D.maxSphereDiameter))
      sphere3d.render(shadesCurrent, !addAllPixels, diameter, x, y, z, mToEllipsoidal, coef, mDeriv, selectedOctant,
          octantPoints);
  }
  ...
  public void plotImage(int x, int y, int z, Image image, JmolRendererInterface jmolRenderer, short bgcolix, int width,
      int height) {
    setColix(bgcolix);
    if (bgcolix == 0)
      argbCurrent = 0;
  ...
  /*
   * private void setRectClip(int x, int y, int width, int height) { // not implemented if (x < 0) x = 0; if (y < 0) y =
   * 0; if (x + width > windowWidth) width = windowWidth - x; if (y + height > windowHeight) height = windowHeight - y;
   * clipX = x; clipY = y; clipWidth = width; clipHeight = height; if (antialiasThisFrame) { clipX *= 2; clipY *= 2;
   * clipWidth *= 2; clipHeight *= 2; } }
   */
  ...
  public void drawDashedLine(int run, int rise, Point3i pointA, Point3i pointB) {
    // measures only
    line3d.plotDashedLine(argbCurrent, !addAllPixels, run, rise, pointA.x, pointA.y, pointA.z, pointB.x, pointB.y,
        pointB.z, true);
  }

  public void drawDottedLine(Point3i pointA, Point3i pointB) {
    // axes, bbcage only
    line3d.plotDashedLine(argbCurrent, !addAllPixels, 2, 1, pointA.x, pointA.y, pointA.z, pointB.x, pointB.y, pointB.z,
        true);
  }

  public void drawLine(int x1, int y1, int z1, int x2, int y2, int z2) {
    // stars
    line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent, !addAllPixels, x1, y1, z1, x2, y2, z2, true);
  }
  ...
  public void drawLine(short colixA, short colixB, int x1, int y1, int z1, int x2, int y2, int z2) {
    ...
    if (colixA == 0 && colixB == 0)
      return;
    line3d.plotLine(argbA, isScreenedA, argbCurrent, !addAllPixels, x1, y1, z1, x2, y2, z2, true);
  }

  public void drawLine(Point3i pointA, Point3i pointB) {
    // draw quadrilateral and hermite
    line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent, !addAllPixels, pointA.x, pointA.y, pointA.z, pointB.x,
        pointB.y, pointB.z, true);
  }
  ...
  public void fillCylinderScreen(byte endcaps, int diameter, int xA, int yA, int zA, int xB, int yB, int zB) {
    ...
    cylinder3d.render(colixCurrent, colixCurrent, !addAllPixels, !addAllPixels, endcaps, diameter, xA, yA, zA, xB, yB,
        zB);
  }

  public void fillCylinderScreen(byte endcaps, int diameter, Point3i screenA, Point3i screenB) {
    // draw
    cylinder3d.render(colixCurrent, colixCurrent, !addAllPixels, !addAllPixels, endcaps, diameter, screenA.x,
  ...
  public void fillCylinderBits(byte endcaps, int diameter, Point3f screenA, Point3f screenB) {
    ...
    cylinder3d.renderBits(colixCurrent, colixCurrent, !addAllPixels, !addAllPixels, endcaps, diameter, screenA.x,
        screenA.y, screenA.z, screenB.x, screenB.y, screenB.z);
  }
  ...
  public void fillConeScreen(byte endcap, int screenDiameter, Point3i screenBase, Point3i screenTip, boolean isBarb) {
    ...
    cylinder3d.renderCone(colixCurrent, !addAllPixels, endcap, screenDiameter, screenBase.x, screenBase.y,
        screenBase.z, screenTip.x, screenTip.y, screenTip.z, false, isBarb);
  }

  public void fillConeSceen(byte endcap, int screenDiameter, Point3f screenBase, Point3f screenTip) {
    // cartoons, rockets
    cylinder3d.renderCone(colixCurrent, !addAllPixels, endcap, screenDiameter, screenBase.x, screenBase.y,
        screenBase.z, screenTip.x, screenTip.y, screenTip.z, true, false);
  }

  public void drawHermite(int tension, Point3i s0, Point3i s1, Point3i s2, Point3i s3) {
  }

  public void drawHermite(boolean fill, boolean border, int tension, Point3i s0, Point3i s1, Point3i s2, Point3i s3,
      Point3i s4, Point3i s5, Point3i s6, Point3i s7, int aspectRatio) {
  }

  public void fillHermite(int tension, int diameterBeg, int diameterMid, int diameterEnd, Point3i s0, Point3i s1,
  ...
  public void drawTriangle(Point3i screenA, short colixA, Point3i screenB, short colixB, Point3i screenC, short colixC,
      int check) {
  ...
  public void drawTriangle(Point3i screenA, Point3i screenB, Point3i screenC, int check) {
    ...
    if ((check & 1) == 1)
      line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent, !addAllPixels, screenA.x, screenA.y, screenA.z,
          screenB.x, screenB.y, screenB.z, true);
    if ((check & 2) == 2)
      line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent, !addAllPixels, screenB.x, screenB.y, screenB.z,
          screenC.x, screenC.y, screenC.z, true);
    if ((check & 4) == 4)
      line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent, !addAllPixels, screenA.x, screenA.y, screenA.z,
          screenC.x, screenC.y, screenC.z, true);
  }
  ...
  /*
   * public void drawfillTriangle(int xA, int yA, int zA, int xB, int yB, int zB, int xC, int yC, int zC) { // sticks --
   * sterochemical wedge notation -- not implemented? line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent,
   * !addAllPixels, xA, yA, zA, xB, yB, zB, true); line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent,
   * !addAllPixels, xA, yA, zA, xC, yC, zC, true); line3d.plotLine(argbCurrent, !addAllPixels, argbCurrent,
   * !addAllPixels, xB, yB, zB, xC, yC, zC, true); triangle3d.fillTriangle(xA, yA, zA, xB, yB, zB, xC, yC, zC, false); }
   */
  public void fillTriangleTwoSided(short normix, int xScreenA, int yScreenA, int zScreenA, int xScreenB, int yScreenB,
      int zScreenB, int xScreenC, int yScreenC, int zScreenC) {
    // polyhedra
  }
  ...
  public void fillTriangle(Point3i screenA, short colixA, short normixA, Point3i screenB, short colixB, short normixB,
      Point3i screenC, short colixC, short normixC, float factor) {
    ...
    if (rgb16Left == null) {
      ...
      while (--count >= 0) {
        if (addAllPixels || (flipflop = !flipflop)) {
          int z = zScaled >> 10;
          if (z >= slab && z <= depth && z < zbuf[offsetPbuf])
            addPixel(offsetPbuf, z, 0xFF000000 | (0 & 0xFF0000) | (0 & 0xFF00) | ((0 >> 8) & 0xFF));
        }
        ++offsetPbuf;
        zScaled += zIncrementScaled;
      }
  ...
  /*
   * final static boolean ENABLE_GOURAUD_STATS = false; static int totalGouraud; static int shortCircuitGouraud;
   * 
   * void plotPixelsUnclipped(int count, int x, int y, int zAtLeft, int zPastRight, Rgb16 rgb16Left, Rgb16 rgb16Right) {
   * // for Triangle3D.fillRaster if (count <= 0) return; int seed = (x << 16) + (y << 1) ^ 0x33333333; // scale the z
   * coordinates; int zScaled = (zAtLeft << 10) + (1 << 9); int dz = zPastRight - zAtLeft; int roundFactor = count / 2;
   * int zIncrementScaled = ((dz << 10) + (dz >= 0 ? roundFactor : -roundFactor)) / count; int offsetPbuf = y * width +
   * x; if (rgb16Left == null) { if (!isTranslucent) { while (--count >= 0) { int z = zScaled >> 10; if (z <
   * zbuf[offsetPbuf]) { zbuf[offsetPbuf] = z; seed = ((seed << 16) + (seed << 1) + seed) & 0x7FFFFFFF; int bits = (seed
   * >> 16) & 0x07; pbuf[offsetPbuf] = (bits == 0 ? argbNoisyDn : (bits == 1 ? argbNoisyUp : argbCurrent)); }
   * ++offsetPbuf; zScaled += zIncrementScaled; } } else { boolean flipflop = ((x ^ y) & 1) != 0; while (--count >= 0) {
   * flipflop = !flipflop; if (flipflop) { int z = zScaled >> 10; if (z < zbuf[offsetPbuf]) { zbuf[offsetPbuf] = z; seed
   * = ((seed << 16) + (seed << 1) + seed) & 0x7FFFFFFF; int bits = (seed >> 16) & 0x07; pbuf[offsetPbuf] = (bits == 0 ?
   * argbNoisyDn : (bits == 1 ? argbNoisyUp : argbCurrent)); } } ++offsetPbuf; zScaled += zIncrementScaled; } } } else {
   * boolean flipflop = ((x ^ y) & 1) != 0; if (ENABLE_GOURAUD_STATS) { ++totalGouraud; int i = count; int j =
   * offsetPbuf; int zMin = zAtLeft < zPastRight ? zAtLeft : zPastRight;
   * 
   * if (!isTranslucent) { for (; zbuf[j] < zMin; ++j) if (--i == 0) { if ((++shortCircuitGouraud % 100000) == 0)
   * Logger.debug("totalGouraud=" + totalGouraud + " shortCircuitGouraud=" + shortCircuitGouraud + " %=" + (100.0 *
   * shortCircuitGouraud / totalGouraud)); return; } } else { if (flipflop) { ++j; if (--i == 0) return; } for (;
   * zbuf[j] < zMin; j += 2) { i -= 2; if (i <= 0) { if ((++shortCircuitGouraud % 100000) == 0)
   * Logger.debug("totalGouraud=" + totalGouraud + " shortCircuitGouraud=" + shortCircuitGouraud + " %=" + (100.0 *
   * shortCircuitGouraud / totalGouraud)); return; } } } }
   * 
   * int rScaled = rgb16Left.rScaled << 8; int rIncrement = ((rgb16Right.rScaled - rgb16Left.rScaled) << 8) / count; int
   * gScaled = rgb16Left.gScaled; int gIncrement = (rgb16Right.gScaled - gScaled) / count; int bScaled =
   * rgb16Left.bScaled; int bIncrement = (rgb16Right.bScaled - bScaled) / count; while (--count >= 0) { if
   * (!isTranslucent || (flipflop = !flipflop)) { int z = zScaled >> 10; if (z < zbuf[offsetPbuf]) { zbuf[offsetPbuf] =
   * z; pbuf[offsetPbuf] = (0xFF000000 | (rScaled & 0xFF0000) | (gScaled & 0xFF00) | ((bScaled >> 8) & 0xFF)); } }
   * ++offsetPbuf; zScaled += zIncrementScaled; rScaled += rIncrement; gScaled += gIncrement; bScaled += bIncrement; } }
   * }
   */
  // /////////////////////////////////
  void plotPixelsUnclipped(int count, int x, int y, int zAtLeft, int zPastRight, Rgb16 rgb16Left, Rgb16 rgb16Right) {
    ...
    if (rgb16Left == null) {
      ...
      while (--count >= 0) {
        if (addAllPixels || (flipflop = !flipflop)) {
          int z = zScaled >> 10;
          if (z < zbuf[offsetPbuf])
            addPixel(offsetPbuf, z, 0xFF000000 | (rScaled & 0xFF0000) | (gScaled & 0xFF00) | ((0 >> 8) & 0xFF));
        }
        ++offsetPbuf;
        zScaled += zIncrementScaled;
        rScaled += rIncrement;
  ...
  /*
   * entries 0 and 1 are reserved and are special inheritance 0 INHERIT_ALL inherits both color and translucency 1
   * INHERIT_COLOR is used to inherit just the color
   * 
   * 
   * 0x8000 changeable flag (elements and isotopes, about 200; negative) 0x7800 translucent flag set
   * 
   * NEW: 0x0000 translucent level 0 (opaque) 0x0800 translucent level 1 0x1000 translucent level 2 0x1800 translucent
   * level 3 0x2000 translucent level 4 0x2800 translucent level 5 0x3000 translucent level 6 0x3800 translucent level 7
   * 0x4000 translucent level 8 (invisible)
   * 
   * 0x0000 inherit color and translucency 0x0001 inherit color; translucency determined by mask 0x0002 special palette
   * ("group", "structure", etc.); translucency by mask
   * 
   * Note that inherited colors and special palettes are not handled here. They could be anything, including totally
   * variable quantities such as distance to an object. So there are two stages of argb color determination from a
   * colix. The special palette flag is only used transiently - just to indicate that the color selected isn't a known
   * color. The actual palette-based colix is saved here, and and the atom or shape's byte paletteID is set as well.
   * 
   * Shapes/ColorManager: responsible for assigning argb colors based on color palettes. These argb colors are then used
   * directly.
   * 
   ...
   * 
   * 
   * 0x0004 black... .... 0x0017 ...gold 0x00?? additional colors used from JavaScript list or specified by user
   * 
   * Bob Hanson 3/2007
   */
  ...
  private final static short applyColorTranslucencyLevel(short colix, float translucentLevel) {
    ...
    int iLevel = (int) (translucentLevel < 1 ? translucentLevel * 256
        : translucentLevel <= 9 ? ((int) (translucentLevel - 1)) << 5 : translucentLevel < 15 ? 8 << 5
            : translucentLevel);
    iLevel = (iLevel >> 5) % 16;
  ...
  public static float getColixTranslucencyFractional(short colix) {
    ...
    return (translevel == -1 ? 0.5f : translevel == 0 ? 0 : translevel == 255 ? 1 : translevel / 256f);
  }
  ...
  public final static short getColixInherited(short myColix, short parentColix) {
    ...
    default:
      ...
      return ((myColix & OPAQUE_MASK) == INHERIT_COLOR ? (short) (parentColix & OPAQUE_MASK | myColix
          & TRANSLUCENT_MASK) : myColix);
    }
  }
  ...
  /****************************************************************
   * changeable colixes give me a short ID and a color, and I will give you a colix later, you can reassign the color if
   * you want Used only for colorManager coloring of elements
   ****************************************************************/
  ...
  void setColorNoisy(int shadeIndex) {
    ...
    argbNoisyUp = shadesCurrent[shadeIndex];
    argbNoisyDn = shadesCurrent[shadeIndex];
  }

  /**
   * used by CartoonRenderer (DNA surface) and GeoSurfaceRenderer (face) to assign a noisy shade to the surface it will
   * render
   * 
   * @param screenA
   * @param screenB
  ...
  private int getShadeIndex(Point3f screenA, Point3f screenB, Point3f screenC) {
  ...
  private final static String[] colorNames = { "black", // 000000
      "pewhite", // ffffff
      "pecyan", // 00ffff
      "pepurple", // d020ff
      "pegreen", // 00ff00
  ...
  private final static int[] colorArgbs = { 0xFF000000, // black
  ...
  public void drawAtom(Atom atom) {
    fillSphere(atom.screenDiameter, atom.screenX, atom.screenY, atom.screenZ);
  }
  ...
  public boolean initializeExporter(String type, Viewer viewer, double privateKey, Graphics3D g3d, Object output) {
    return false;
  }
